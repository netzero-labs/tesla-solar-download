...
if [ "$1" == "--refresh" ]; then
    ...
    curl -s -X POST -H 'Content-Type: application/x-www-form-urlencoded' --data-urlencode 'grant_type=refresh_token' \
...
TESLA_BASE_URL=$(curl -s -H "Authorization: Bearer $TESLA_API_TOKEN" https://fleet-api.prd.na.vn.cloud.tesla.com/api/1/users/region | jq -r .response.fleet_api_base_url)
echo "Tesla base url: $TESLA_BASE_URL"

# Get the energy site id.
TESLA_SITE_ID=$(curl -s -H "Authorization: Bearer $TESLA_API_TOKEN" $TESLA_BASE_URL/api/1/products | jq -r .response[0].energy_site_id)
echo "Tesla energy site id: $TESLA_SITE_ID"

# Get site info.
curl -s -H "Authorization: Bearer $TESLA_API_TOKEN" $TESLA_BASE_URL/api/1/energy_sites/$TESLA_SITE_ID/site_info | jq

# To set backup reserve to 50%:
#curl -s -H "X-Tesla-User-Agent: TeslaApp/4.29.0" -H "Authorization: Bearer $TESLA_API_TOKEN" --json '{"backup_reserve_percent": 50}' $TESLA_BASE_URL/api/1/energy_sites/$TESLA_SITE_ID/backup
